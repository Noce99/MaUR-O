...
- ## Step 4: Elevation Value Assignation
	...
	- Now let's define the following procedure (**Elevation Proliferation**) on a contour **C**. A single rain drop is not trusted to decide a hit contour's own fate by itself: a drop whose own direction happens to be close to perpendicular to the hit contour's own gravity there is exactly the case where the accordance/discordance sign is most sensitive to noise, so every drop that reaches a given contour during this one procedure call instead casts a **vote**, and only once every drop has evaporated do we turn each hit contour's own tally into a decision.
		...
		5) If this contour already has a defined **elevation_height** (we call it **AC**), we compare **expected_elevation_height** with **AC**'s own **elevation_height**. If abs(**expected_elevation_height**) <= abs(**elevation_height**) no operation is needed. Otherwise, abs(**expected_elevation_height**) > abs(**elevation_height**), we have to remove **AC** (and all its children) from **T** setting to undefined all the **elevation_height** and **empty_progeny**==false of the removed contours -- only **AC** itself is added back (below); every other removed contour (its former children, grandchildren, ...) is simply dropped out of **T** rather than reattached anywhere, and will only rejoin **T** later if some other proliferation happens to hit it again. After that we will add **AC** again to **T** as child of **C** setting its value to **expected_elevation_height**. It should not be possible that **C** is a descendant of **AC**, since removing **AC**'s subtree would then also remove **C** -- the very contour currently being proliferated -- out from under itself; if this is ever found to happen anyway, the algorithm should crash with an error naming both **C** and **AC**, the same way the "cannot happen" case is handled elsewhere in this document.
	...
	- After that we do the following.
		...
		```
	- In case some contours still have an undefined **elevation_height** we warning the user and we delete them.
...
- ## Visualization
	...
	- `07_<map_name>_step4.svg`: like `06_<map_name>_step3_anti_rain.svg`'s own contour-line layer (no rain-drop dots/paths, no gravity arrows), but every contour with a resolved **elevation_height** is instead drawn in a color interpolated along a blue (lowest **elevation_height** across the whole map) to red (highest) gradient, with a small black text label reading `"<its own index into the Contours vector>: <elevation_height>"` at the point exactly halfway along its own line by arc length (not its geometric centroid, which for a closed ring sits near the ring's own enclosed area instead -- concentric elevation contours, the ordinary case for a real hill, would otherwise all collapse their labels into the same illegible spot near the hill's own center regardless of how differently sized the rings themselves are). On top, one thin gray line per **T** edge, from the parent contour's own label point to the child's, showing how elevation propagated; a small black square is drawn on every contour whose own **empty_progeny** is `true`, marking a stalled proliferation leaf. Any contour still without an **elevation_height** -- whether Step 4 finished and dropped it (per its own warning), or a run stopped early on the "should not be possible" error below and simply never got to it -- is drawn in solid gray instead, with no gradient color and no **T** edge (it was never added to **T**), but still labeled with just its own bare index, so a contour named in a warning or in that error can always be found in the picture. If the run stopped on that "should not be possible" error, the exact Hot (Anti) Rain Drop that triggered it is drawn on top of everything else, in orange, thicker than any contour's own line: its whole path, a hollow circle at its own source, and a filled one at the fatal evaporation point -- so the drop that caused the crash, not just the two contours' own indices the error message names, is itself visible without having to re-derive it from the message by hand.
- ## Appendix 1
