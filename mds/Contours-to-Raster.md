...
- ## Assumptions
  collapsed:: true
	...
	- 2) Wherever a Form Line runs between two ordinary contours, it sits exactly halfway between them in elevation -- so crossing into or out of it always represents half the equidistance, regardless of how far apart its two neighboring ordinary contours actually are.
- ## Parameters
	...
	- **rain_drop_starting_voting_hysteresis** (pure number, counted in **rain_drop_step**-sized steps) — a Cold *or Hot* rain drop is exempt from evaporating on re-crossing its own starting contour within this many steps of being created (see the [Rain Drop Production Definition](6aa1c9d2-3f4a-4b1e-8a6c-9e2d5f7b1a3c)); Cold-only, separately, a Cold drop is also exempt from evaporating on re-crossing any other contour it has already voted for within this many steps of *that vote* -- a Hot drop has no such second exemption, since it never votes. Should be just large enough (a handful of steps) to carry the drop clear of whichever applies; since each exemption is measured from its own reference point (creation, or the vote itself), it does not need to be sized relative to the distance to neighboring contours or to how far into the drop's path a vote happens to fall.
	...
	- **elevation_vote_min_total_weight** (pure number) — the minimum combined confidence weight (accordance weight plus discordance weight, from every Hot Rain/Anti Rain Drop that hits a given contour during one Elevation/Anti Elevation Proliferation call) before Step 4's own vote will decide that contour's `elevation_height` in this call at all. Below this there simply isn't enough evidence to trust either side yet, and the contour is left undecided for a later call to resolve instead (Step 4). Should scale with **sources_per_contour_segment**: fewer sources per segment means less total weight is ever achievable for a given hit, so too high a value here could leave a contour permanently undecided even when every drop that reached it agreed.
	- **elevation_vote_min_margin** (pure number, same units as **elevation_vote_min_total_weight**) — the minimum margin the winning side (accordance or discordance) of Step 4's own vote must lead the losing side by before being trusted without a warning. Below this the vote is still decided (accordance wins an exact tie) but flagged, naming the contour, the pass, and both weights (Step 4).
	...
	- **gravity_gaussian_kernel_size** (pixels) — the window size of the Gaussian kernel Step 5's own per-pixel Gravity Direction sub-step smooths its direction field with once gap-filling has run: a `kernel_size x kernel_size` window (radius `(kernel_size - 1) / 2`), standard deviation `radius / 3.0` (the window's own edge sitting at roughly three standard deviations, the usual rule of thumb for how large a Gaussian window needs to be to capture essentially all of the kernel's own mass). Only a pixel that already has a direction of its own is smoothed, averaged only over its own same-defined neighbors (not diluted by an out-of-bound or still-undefined one); a pixel with no direction stays undefined. `1` (or `0`) turns smoothing off outright.
	- **elevation_gaussian_kernel_size** (pixels) — the same, for Step 5's own final per-pixel elevation (the E2V) instead of its gravity direction: smoothed the same way, once its own gap-filling has run, independently of **gravity_gaussian_kernel_size**. `1` (or `0`) turns smoothing off outright.
- ## Rain Drop Production Definition
  ...
  collapsed:: true
	...
	- **Stepping and hit detection** (shared by all four variants): we let each rain drop step **rain_drop_step** meters at a time. To check what a step crosses, see [Appendix 4](6aa157fd-862a-4e2d-9b2c-2184a96494cc), which reports the first of: a specific contour (by index), an out-of-bound (`1`) pixel, or a high-density (`3`) pixel that the step touches, or reports the step as clear if none of these occurs -- without excluding the drop's own source contour, which every variant instead handles in its own way below, since a source contour concave enough to send the drop back across its own boundary later in its life must still be able to evaporate it then, the same as any other contour would (only its own *origin*, right at the start, needs protecting). A **Hot** drop (below) is exempt from evaporating on its own starting contour only within **rain_drop_starting_voting_hysteresis** steps of its own creation -- the same window a **Cold** drop's own starting-contour exemption uses, just without a per-vote one of its own, since a Hot drop never votes. A **Reverse Cold** drop needs no such window at all: its own source contour is, by construction, always still undefined when it runs, and crossing a still-undefined contour is already a silent pass for it regardless of which contour that is (see below).
	- **Temperature — Cold vs Hot** (governs only when a drop evaporates; independent of Direction):
		...
		- A **Hot** rain drop evaporates when it crosses *any* contour at all — gravity defined or not — or when it crosses a high-density (`3`) pixel, or when it crosses an out-of-bound (`1`) pixel, subject to the same **rain_drop_starting_voting_hysteresis** starting-contour exemption a Cold drop gets (see "Stepping and hit detection" above). **undefined_gravity_vote_threshold** is Cold-only: a Hot rain drop never casts *this*, Step 3's own gravity-direction vote, so it does not apply to it -- Step 4's own, entirely separate accordance/discordance vote on a Hot drop's own evaporation (**elevation_vote_min_total_weight**/**elevation_vote_min_margin**, below) is a different mechanism with its own thresholds.
...
- ## Step 1: Extrapolate Elevation Information from an .omap
	...
	- We instantiate a **Vector** of **Contours** for each contour (or form line) in the .omap file. An idea of the struct definitions is shown:
	  ```rust
	  struct GravityVotes{
	  ...
	  struct Contour{
	      lwg: LineWithGravity,
	      /// How much elevation crossing this contour represents, in
	      /// equidistances: 1.0 for an ordinary [Index] Contour, 0.5 for a
	      /// Form Line (Assumption 2).
	      step: f64,
	      elevation_height: Option<f64>,
		  empty_progeny: bool // Should be false when a Contour is instanciated
	  }
	  ```
...
- ## Step 3: Gravity Definition with Rain
	...
	- For every contour still without a gravity direction once both passes above have finished, we run a **Reverse Cold Rain Drop Production** and a **Reverse Cold Anti Rain Drop Production** from it. Since the source itself has no gravity yet, source placement uses an arbitrary placeholder side for it, the same trick Step 1's own Hot Rain/Anti Rain Drop Production already uses before any contour has a real gravity direction at all — running both perpendicular directions from every source point stands in for the Rain/Anti Rain distinction here, regardless of which side happened to get the placeholder label.
		- **Reverse Cold** evaporates on exactly the same three things **Cold** does — an out-of-bound pixel, a high-density pixel, or a contour whose gravity is already defined — but differs in what happens at each: crossing a contour whose gravity is *not yet* defined is a silent pass, never a vote (there is no real gravity at the source yet to judge a side from, unlike Cold's own undefined-contour crossing); and evaporating on a contour whose gravity *is* already defined casts one vote, not on the contour it hit, but on its own *source* contour instead. That vote's side is decided by comparing the drop's own direction of travel against the hit contour's gravity direction at the point it hit it, exactly the accordance/discordance comparison Step 4 already defines (below): in accordance, the drop's own direction *is* the source's real downhill side, by the same regional downhill-sense reasoning as everywhere else in this section; in discordance, the source's real downhill is the opposite direction instead. Because it never votes on what it merely passes through, Reverse Cold does not need `rain_drop_starting_voting_hysteresis` at all: a drop can cast at most one vote in its whole lifetime, right before it evaporates, so there is never a re-crossing to protect.
		- A high-density pixel is also worth a vote, but only if it is a Jump's own stamped area, not a genuine two-contour rasterization conflict (Step 1): unlike a real conflict, which has no gravity to offer, a Jump's own gravity is real and already known (read directly from the .omap tick, Step 1), so a high-density pixel found to fall inside a Jump's own buffered polygon is treated exactly like evaporating on a contour with defined gravity above, using the Jump's own gravity direction in place of a hit contour's. A high-density pixel outside every Jump's own polygon is a genuine conflict and remains a dead end, exactly as for every other rain-drop variant.
		- Once both of these have finished, we again define the gravity of every contour that received a vote from them, the same way as above. It is not only possible but expected that some contours may still be undefined at this point: a small enough **sources_per_contour_segment**, so that some drop's own straight-line path from a given source point actually reaches a contour with defined gravity (or a Jump) before running into an out-of-bound or a genuinely conflicted high-density pixel, is what closes that gap — not a further round of this pass, since a single Reverse Cold drop already searches straight through as many still-undefined contours as its path crosses before giving up.
	- Any contour still without a gravity direction once every pass above has finished is dropped rather than treated as a failure: the algorithm no longer assumes this "cannot happen." Instead, a **warning** is reported for each one, naming that contour's own length in meters, so a genuinely unreachable or too-sparsely-evidenced contour doesn't abort an otherwise-good run — whoever reviews the warnings can judge from its length alone whether it was worth digitizing in the first place.
- ## Step 4: Elevation Value Assignation
	...
	- Step 3 can leave a contour dropped -- still without a gravity direction, only warned about, not treated as a failure -- while its own pixels are still drawn into the Contour Raster under its own index. Before this step's own assumption above can hold for every contour a rain drop might still physically hit, every such dropped contour's own Contour Raster pixels are first reset to **NO_CONTOUR_IN_BOUND** (`2`), the same way Step 1's own Growing Process sweeps a **TEMPORARY_CONTOUR** pixel back to `2` once it is no longer needed -- so a rain drop passes through where a dropped contour used to be exactly as if it had never been digitized at all.
	- First of all we create a Tree (the Data Structure) of contours that we will call **T**. **T** should keep track of the deepness of the elements added to it.
	- Now let's define the following procedure (**Elevation Proliferation**) on a contour **C**. A single rain drop is not trusted to decide a hit contour's own fate by itself: a drop whose own direction happens to be close to perpendicular to the hit contour's own gravity there is exactly the case where the accordance/discordance sign is most sensitive to noise, so every drop that reaches a given contour during this one procedure call instead casts a **vote**, and only once every drop has evaporated do we turn each hit contour's own tally into a decision.
		1) We run a **Hot Rain Drop Production** from **C** (see [Rain Drop Production Definition](6aa1c9d2-3f4a-4b1e-8a6c-9e2d5f7b1a3c)): since every contour already has its gravity defined by this point, a Hot rain drop's evaporation always happens on some contour (or on high density, or out of bound, which is a non-operation).
		2) When a rain drop evaporates by hitting another contour, we compute the **accordance** or **discordance** of the rain drop's own direction and that contour's gravity direction there, exactly as before, but instead of acting on it immediately we cast a **vote** for that side on that specific hit contour, weighted by **|cos(θ)|** between the drop's own direction and the hit's own gravity vector at the point it hit it (**1** when the two are exactly aligned or exactly anti-aligned -- a confident read either way -- fading to **0** exactly at the perpendicular case). Each hit contour accumulates its own two running totals (accordance weight, discordance weight) across every drop that reaches it during this one procedure call.
		3) Once every drop from this call has evaporated, for each contour that received at least one vote this call: if the combined weight of both sides together does not clear **elevation_vote_min_total_weight**, this contour is left undecided for now -- there simply isn't enough evidence yet, and a later call (from a different, better-placed contour) may still resolve it. Otherwise the side with the larger weight wins (an exact tie is won by accordance), and if the winning side's own margin over the losing one does not clear **elevation_vote_min_margin** a **warning** is raised naming the contour, the pass, and both weights, though the vote is still decided. From here on the winning side is used exactly as a single drop's own accordance/discordance would have been used before: in case of **accordance** we set the height as the **C** height minus **step**, in case of **discordance** we set it as the same height value of **C** -- call this value **expected_elevation_height**. **step** is `min(C.step, hit.step)` (Assumption 2): `1.0` unless either **C** or the hit contour is a Form Line, in which case `0.5` -- a Form Line always sits exactly halfway, in elevation, between the ordinary contours on either side of it, so a hop into or out of one is always a half step, and only a hop between two ordinary contours (no Form Line drawn between them) is a full one.
		4) If this contour has no defined **elevation_height** yet, we add it as a child of **C** in **T** and set its **elevation_height** to **expected_elevation_height**.
		5) If this contour already has a defined **elevation_height** (we call it **AC**), we compare **expected_elevation_height** with **AC**'s own **elevation_height**. If abs(**expected_elevation_height**) <= abs(**elevation_height**) no operation is needed. Otherwise, abs(**expected_elevation_height**) > abs(**elevation_height**), we have to remove **AC** (and all its children) from **T** setting to undefined all the **elevation_height** and **empty_progeny**==false of the removed contours -- only **AC** itself is added back (below); every other removed contour (its former children, grandchildren, ...) is simply dropped out of **T** rather than reattached anywhere, and will only rejoin **T** later if some other proliferation happens to hit it again. After that we will add **AC** again to **T** as child of **C** setting its value to **expected_elevation_height**. Ordinarily **C** cannot be a descendant of **AC**, since removing **AC**'s subtree would then also remove **C** -- the very contour currently being proliferated -- out from under itself. In practice this can still happen when a hole Step 1's Growing Process left unfilled (an unresolved Flying End, or a dropped zero-length contour) lets a Hot (Anti) Rain Drop pass through where a fully closed contour set would have blocked it, reaching a contour it structurally shouldn't be able to. When this is found to happen, the eviction is voided instead of crashing: a **warning** is raised naming both **C** and **AC**, the pass, both elevation_height values, and the vote that triggered it, and **AC** is left exactly as it was -- no height change, no re-parenting. This call's vote on **AC** is simply discarded, the same as if it had lost the vote outright.
		6) The procedure ends once every contour's own vote from this call has been decided (point 3) and acted on (points 4/5), returning the number of contours newly added, or evicted and re-parented, as children of **C**.
	- Let's also define the **Anti Elevation Proliferation** that works like **Elevation Proliferation** but running a **Hot Anti Rain Drop Production** instead, with the accordance/discordance rule flipped: in case of **accordance** we set the height as the same value of **C**, and in case of **discordance** we set it as the **C** height plus **step** (the same **step** as above). This flip matters: an Anti Rain drop already travels *against* **C**'s own downhill, so the *ordinary* case of genuinely reaching higher ground is discordance with the hit contour's own gravity (its downhill normally points back down toward **C**, opposite the drop's own uphill travel) -- reusing Elevation Proliferation's own mapping unflipped would instead treat that ordinary case as "no higher than **C**" and only ever produce heights at or below the root's own.
	- Let's also define the **Next Proliferator Selection** procedure that, given **T**, select one of the leaves with **empty_progeny**==false. More preciselly it select the leaves with the smaller depth inside the tree. In case of multiple leaves of equal tree depth it chooses randomly among them. If there are no leaves with **empty_progeny**==false it return None.
	- We randomly choose a contour **C** and we add it as the root of **T**. It's altitude (elevation_height) is set to 0.
	- After that we do the following.
		```
		while C = Next_Proliferator_Selection(T) is not None:
			childrens = Elevation_Proliferation(T, C)
			anti_childrens = Anti_Elevation_Proliferation(T, C)
			if childrens == 0 and anti_childrens == 0:
				C.empty_progeny = True
		```
	- In case some contours still have an undefined **elevation_height** we warning the user and we delete them (also from the 2D Contour Vector).
- ## Step 5: Final Tiff Computation
	- Given every contour's own **elevation_height** (Step 4), we build a second 2D Vector, the **Elevation 2D Vector** (**E2V**), covering the same extent at the same **rasterization_px_size** pixel size as the Contour Raster (**C2V**: **Contour 2D Vector**) -- so a pixel at raster coordinates (x, y) always refers to the same ground location in both.
	- Every E2V pixel whose corresponding C2V pixel is a contour (value >= **CONTOUR_0_MATRIX_VALUE**) is seeded with that contour's own **elevation_height**.
	- From every contour, we then run an **Elevation Fill Rain Drop Production**: a fifth variant, reusing the [Rain Drop Production Definition](6aa1c9d2-3f4a-4b1e-8a6c-9e2d5f7b1a3c)'s own Source placement and Source direction (Rain only -- no Anti Rain counterpart, since a contour's own downhill drops already cover the band down to its next lower neighbor, and that neighbor's own downhill drops cover the band below *it*, so every band between two adjacent contours is covered by exactly one of the two) and the same **rain_drop_step**-at-a-time stepping, but with its own evaporation rule instead of Cold/Hot/Reverse Cold's:
		- An out-of-bound (`1`) pixel evaporates the drop with no further effect (a no-op): nothing this drop has touched gets written to the E2V.
		- A high-density (`3`) pixel evaporates the drop too -- unlike it passing straight through undefined/no-contour-in-bound pixels, high density is a real, if unresolved, obstacle (two or more contours bunched closer together than **rasterization_px_size** can tell apart, Step 1): sailing straight through it can leapfrog a whole nested cluster of real, distinct contours it hides, landing many bands further than it should over a very short track. Since there's no real contour index to read an elevation from there, we suppose a fallback: the starting contour's own **elevation_height** minus its own **step** -- the same "one step further downhill" assumption Step 4's own accordance case makes (see below), including its own Form Line half-step.
		- Any contour evaporates the drop -- including its own starting one, subject to the same **rain_drop_starting_voting_hysteresis** starting-contour exemption a Hot drop gets (see the [Rain Drop Production Definition](6aa1c9d2-3f4a-4b1e-8a6c-9e2d5f7b1a3c)'s own "Stepping and hit detection"): within that window a re-crossing of its own source contour is a silent pass, past it the drop evaporates on it exactly as on any other contour.
	- Since a rain drop's own direction never changes once it starts (Rain Drop Production Definition), its whole track from source to evaporation is a single straight segment, so every pixel that segment's own Appendix 4 walk touches gets a value: **value = (dB \* eA + dA \* eB) / (dA + dB)**, where **eA** is the elevation of the contour the drop started from, **eB** the elevation of the contour it evaporated on (or the high-density fallback above), and **dA**/**dB** the straight-line distance from that pixel to the track's start/end respectively -- note **eA** is weighted by **dB** and **eB** by **dA**, the distance to the *other* end, not its own, so a pixel right next to the start actually lands close to **eA** rather than **eB**.
	- A pixel is very likely to be written by more than one drop's own track, and not every track deserves equal trust: a short track reflects genuinely close-by terrain, while a rare long one (an open area with no closer contour along that exact ray) says much less about everything in between it blends over. So rather than a plain mean, each E2V pixel keeps a running weighted-sum/weight-total pair, updated as each drop's track is committed with weight **1 / track_len** (the drop's own whole straight-line length, the same for every pixel that one track touches); its final value is the weighted sum divided by the weight total. A contour's own seed value, and a gap-filling round's own neighbor average (below), each count with weight `1`.
	- Once every contour's own Elevation Fill Rain Drop Production has run, some E2V pixels will likely still be unset -- most of all, the interior of an innermost (hilltop) contour's own ring and any area past a map-edge contour's outward side, neither of which any drop's own downhill track ever reaches. We fill these with an iterative, 8-connected front propagation: each round, every still-unset pixel with at least one *set* 8-neighbour (from the *previous* round's own state, not updated in place, so the result never depends on scan order) is set to the plain average of those neighbours; this repeats until a round sets nothing. This is deliberately the simplest fill that gets every reachable pixel set at all -- it settles to roughly the value of the nearest contour it propagated from rather than extrapolating a slope into a large untouched area (a hilltop's own true peak, for instance) -- and is meant to be swapped later for something that does extrapolate (e.g. a bounded Laplace relaxation) without touching anything upstream of it.
	- Every value the E2V ever holds, from the initial seeding through both fills above, stays in the same unit **elevation_height** itself is in: a count of bands (in halves, once a Form Line is involved -- Assumption 2) relative to Step 4's own arbitrary zero contour, not meters. Only once the E2V is fully resolved, right before it is written out as the final TIFF, is each pixel multiplied by **equidistance** (meters, an argument of the `contours_to_raster` executable itself rather than `--config`, default `5.0`) -- the real vertical distance between two adjacent contour lines on the source map -- turning that band count into an actual elevation in meters, still relative to that same unknown baseline (**Contours-to-Raster.md**'s own opening line). This one multiplication is the only place **equidistance** is ever used: it plays no part in any of Steps 1 through 5's own band-unit arithmetic (deciding gravity, voting on **elevation_height**, weighting a drop's track, averaging a gap-filled neighbourhood), all of which is scale-invariant in the first place.
- ## Visualization
	- For a human check of each step, I would like to see each step, one after the other, as an SVG file. The files are numbered in the order they're produced: `00_<map_name>_step1.svg`, `01_<map_name>_step1_close_search.svg`, `02_<map_name>_step1_growing_seeking.svg`, `03_<map_name>_step1_growing_matching.svg`, `04_<map_name>_step2.svg`, `05_<map_name>_step3_rain.svg`, `06_<map_name>_step3_anti_rain.svg`, `07_<map_name>_step4.svg`.
	...
	- `07_<map_name>_step4.svg`: like `06_<map_name>_step3_anti_rain.svg`'s own contour-line layer (no rain-drop dots/paths, no gravity arrows), but every contour with a resolved **elevation_height** is instead drawn in a color interpolated along a blue (lowest **elevation_height** across the whole map) to red (highest) gradient, with a small black text label reading `"<its own index into the Contours vector>: <elevation_height>"` at the point exactly halfway along its own line by arc length (not its geometric centroid, which for a closed ring sits near the ring's own enclosed area instead -- concentric elevation contours, the ordinary case for a real hill, would otherwise all collapse their labels into the same illegible spot near the hill's own center regardless of how differently sized the rings themselves are). On top, one thin gray line per **T** edge, from the parent contour's own label point to the child's, showing how elevation propagated; a small black square is drawn on every contour whose own **empty_progeny** is `true`, marking a stalled proliferation leaf. Any contour still without an **elevation_height** once Step 4 finishes -- dropped per its own warning, no proliferation call ever having reached it -- is drawn in solid gray instead, with no gradient color and no **T** edge (it was never added to **T**), but still labeled with just its own bare index, so a contour named in any warning can always be found in the picture.
- ## Appendix 1
...
- ## Appendix 4
  ...
  collapsed:: true
	...
	- The function optionally takes a single contour index to exclude from this check entirely (reported the same as `2`, never as a hit) -- used by callers with their own, unrelated reason to ignore one specific contour throughout a whole check (Step 1's Growing Process, checking whether a straight segment between two Flying Ends crosses a third contour, excludes the Flying End's own). The Rain Drop Production Definition's own rain drops do **not** use this to protect a drop's own source contour, even though naively that looks like the same problem (a drop starting right on its own source's pixel would otherwise evaporate against its own origin on its very first step): that exclusion needs to end once the drop is genuinely clear of its own origin, not persist for the drop's whole lifetime, since a source contour concave enough to send the drop back across its own boundary later on must still be able to evaporate it then, the same as any other contour would. That is a *time*-limited exemption, not describable by this appendix's own all-or-nothing exclusion -- so it belongs downstream, in the Rain Drop Production Definition itself (see its own "Stepping and hit detection").
