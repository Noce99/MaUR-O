...
- ## Step 5: Final Tiff Computation
	...
	- Every value the E2V ever holds, from the initial seeding through both fills above, stays in the same unit **elevation_height** itself is in: an integer count of bands relative to Step 4's own arbitrary zero contour, not meters. Only once the E2V is fully resolved, right before it is written out as the final TIFF, is each pixel multiplied by **equidistance** (meters, an argument of the `contours_to_raster` executable itself rather than `--config`, default `5.0`) -- the real vertical distance between two adjacent contour lines on the source map -- turning that band count into an actual elevation in meters, still relative to that same unknown baseline (**Contours-to-Raster.md**'s own opening line). This one multiplication is the only place **equidistance** is ever used: it plays no part in any of Steps 1 through 5's own band-unit arithmetic (deciding gravity, voting on **elevation_height**, weighting a drop's track, averaging a gap-filled neighbourhood), all of which is scale-invariant in the first place.
- ## Visualization
