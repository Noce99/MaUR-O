...
- ## Rain Drop Production Definition
  ...
  collapsed:: true
	...
	- **Temperature — Cold vs Hot** (governs only when a drop evaporates; independent of Direction):
		...
		- A **Hot** rain drop evaporates when it crosses *any* contour at all — gravity defined or not — or when it crosses a high-density (`3`) pixel, or when it crosses an out-of-bound (`1`) pixel, subject to the same **rain_drop_starting_voting_hysteresis** starting-contour exemption a Cold drop gets (see "Stepping and hit detection" above). **undefined_gravity_vote_threshold** is Cold-only: a Hot rain drop never votes, so it does not apply to it.
...
- ## Step 4: Elevation Value Assignation
	...
	- Now let's define the following procedure (**Elevation Proliferation**) on a contour **C**. 
		...
		2) When a rain drop evaporates by hitting another contour **without a defined elevation_height**, we add that contour as a child of **C** in **T** and we set that contour's **elevation_height** based on the **accordance** or **discordance** of the rain drop's own direction and that contour's gravity direction. In case of **accordance** we set the height as the **C** height minus 1. In case of **discordance** we set it as the same height value of **C**.
		3) When a rain drop evaporates by hitting another contour **with a defined elevation_height** (we call it **AC**), we compute as above what is the value that we expect that contour to have (let's call it **expected_elevation_height**) and we compare that value with the hitted contour **elevation_height**. If abs(**expected_elevation_height**) <= abs(**elevation_height**) no operation is needed. Otherwise, abs(**expected_elevation_height**) > abs(**elevation_height**), we have to remove **AC** (and all its children) from **T** setting to undefined all the **elevation_height** and **empty_progeny**==false of the removed contours -- only **AC** itself is added back (below); every other removed contour (its former children, grandchildren, ...) is simply dropped out of **T** rather than reattached anywhere, and will only rejoin **T** later if some other proliferation happens to hit it again. After that we will add **AC** again to **T** as child of **C** setting its value to **expected_elevation_height**. It should not be possible that **C** is a descendant of **AC**, since removing **AC**'s subtree would then also remove **C** -- the very contour currently being proliferated -- out from under itself; if this is ever found to happen anyway, the algorithm should crash with an error naming both **C** and **AC**, the same way the "cannot happen" case is handled elsewhere in this document.
		4) When all rain drops has evaporated the procedure end returning the number of children added to **C** during its execution.
